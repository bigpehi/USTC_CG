...
ParameterizeCotangentCircle::ParameterizeCotangentCircle(Ptr<TriMesh> triMesh)
	: heMesh(make_shared<HEMesh<V>>())
{
	Init(triMesh);
}

void ParameterizeCotangentCircle::Clear() {
	heMesh->Clear();
	triMesh = nullptr;
}

bool ParameterizeCotangentCircle::Init(Ptr<TriMesh> triMesh) {
	Clear();
...
bool ParameterizeCotangentCircle::Run() {
...
void Ubpa::ParameterizeCotangentCircle::Parameterization()
{
	// step1：检测边界
	// step2：固定边界点
	Fix_boundary();
...
void Ubpa::ParameterizeCotangentCircle::Fix_boundary()
{
...
void Ubpa::ParameterizeCotangentCircle::Create_equation()
{
	size_t nV = heMesh->NumVertices(); // mesh的点数
...
void Ubpa::ParameterizeCotangentCircle::Solve_equation()
{
...
void Ubpa::ParameterizeCotangentCircle::Update_vetecies()
{
	size_t nV = heMesh->NumVertices(); // mesh的点数
...
void Ubpa::ParameterizeCotangentCircle::Add_texture()
{
	texture_flag = true;
	size_t nV = heMesh->NumVertices(); // mesh的点数
	for (size_t i = 0; i < nV; i++) {
