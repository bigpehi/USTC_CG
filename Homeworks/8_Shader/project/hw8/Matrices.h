...
namespace GLMath
{
    ...
    // | c f i |   Z        cX + fY + iZ                         i=(1,0,0) j=(0,1,0) k=(0,0,1)
    // m*n   n*p   ==  m*p   m*n  n*1   m*1
    // 3 * 2   2*1  --->  3*1
    // ABx ?   f(g(x))
    ...
    //将3d物体渲染到屏幕上的步骤：
    // 1) 将准备拍摄的对象移动到场景中的必要位置上（模型变换，model transform）
    // 2) 将相机移动到准备拍摄的位置，将摄像机对准某个方向(视图变换，view transform)
    // 3) 设置相机的焦距（投影变换,projection transform）
    // 4) 拍摄照片（应用变换结果）
    // 5) 对结果图像就行拉伸或者挤压，将它变换到需要的图片大小（视口变换,viewport transform）
